...
module.exports = BaseClass.extend({
  ...
  getNewArtifactPath: function(type, template, name, destination) {
    ...
    //with modules separation, we dont want artifacts directly in modules folders
    if( this.cli.getOption('moduleSeparation') && !this.cli.getOption('nestedArtifacts') ) {
      if( path.basename(destination) === 'modules' ) {
        destination = path.resolve(destination+'/../');
      }
    }
  ...
  getNewModulePath: function(type, name, destination) {
    ...
    //determine the destination
    if( this.cli.getOption('moduleSeparation') && path.basename(destination) !== 'modules' ) {      
      destination = path.resolve(destination + '/' + type + 's/' + name);      
    } else {
      destination = path.resolve(destination + '/' + name);
    }
