...
module.exports = BaseClass.extend({
  ...
  projectRoot: function(searchPath) {
    ...
    if (fs.existsSync(path.resolve(path.join(searchPath, 'package.json')))) {
      var pkg = require(path.resolve(path.join(searchPath, 'package.json')));

      if (!pkg.component) {
        this.cachedProjectRoot = searchPath;
        return searchPath;
      } else {
        parentPath = path.resolve(path.join(searchPath, '..'));
      ...
      // exit if we're at the system root to prevent stack overflow
      if( parentPath === systemRoot) {
        return false;
      }
  ...
  projectName: function() {
    ...
    return require(path.resolve(projectRoot, 'package.json')).name;
  },
