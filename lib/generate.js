...
module.exports = BaseClass.extend({
  ...
  createLibrary: function(template, name, destination) {

    var self = this;

    console.log(chalk.cyan(':: creating new library:'), chalk.white(name) + chalk.gray(' ('+path.resolve(destination)+')'));

    if( this.templates.get('library', template) ) {

      this.template('library', template)
        .context({name: name})
        .dest(destination)
        .then(function() {
          if(self.templates.get('library', template).done ) {
            self.templates.get('library', template).done(name, destination);
          }
        });
  ...
  createArtifact: function(type, template, name, destination, callback) {
    ...
    this.template(type, template)
      ...
      .then(function () {
        ...
        var importType = type;
        if( type === 'component' || type === 'view' || type === 'directive' && template === 'element' ) {
          importType = 'module';
        }
    ...
    var worker = {
      ...
      data: {
        cli: {
          ...
          options: merge(this.cli.config.config.options, this.cli.request.request.options)
        }
      },
